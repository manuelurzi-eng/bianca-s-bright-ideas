...
// Elenco letto dalla tabella Team di Airtable (o mock in demo).
export default function Entry({ entryPeople }) {
  return (
    <div style={css('position:fixed;inset:0;z-index:100;background:var(--plum-900);color:var(--on-dark-1);display:flex;align-items:center;justify-content:center;padding:40px;overflow:auto')}>
      <div style={css('width:100%;max-width:560px')}>
        ...
        <h1 style={css('margin:0 0 6px;font-weight:600;font-size:28px;letter-spacing:-.02em')}>Chi sei?</h1>
        <p style={css('margin:0 0 8px;font-size:14px;color:var(--on-dark-2);line-height:1.5')}>Strumento interno protetto da password unica. Scegli il tuo profilo: imposta la vista e il filtro dei clienti. Non è una barriera di sicurezza — solo una selezione di vista.</p>
        ...
        <p style={css('margin:22px 0 0;font-size:12px;color:var(--on-dark-3);line-height:1.5')}>L'elenco è letto dalla tabella <b style={css('color:var(--on-dark-2)')}>Team</b> di Airtable. Aggiungere una PM = aggiungerla al Team e assegnarle clienti; la sua vista esiste senza toccare codice.</p>
      </div>
    </div>
  );
}
